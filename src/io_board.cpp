...
extern uint32_t test1khz_start[];
extern uint32_t test1khz_end[];

extern uint32_t bell_start[];
extern uint32_t bell_end[];

extern uint32_t door_chime_start[];
extern uint32_t door_chime_end[];

extern uint32_t horn_start[];
extern uint32_t horn_end[];
...
int main() {
  ...
  dma_channel_set_trans_count(audio_dma_channel, horn_end - horn_start - 1, false);
  volatile void* audio_write_addr = &pwm_hw->slice[audio_slice].cc;
...
void button_callback(uint gpio, uint32_t mask) {
  if (gpio == BUTTON_PIN) {
    if (mask & GPIO_IRQ_EDGE_RISE) {
      ...
      dma_channel_set_read_addr(audio_dma_channel, horn_start, true);
